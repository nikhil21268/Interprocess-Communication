...
void getRandStr(char random_string[][12]) {        
    srand (time(NULL));                            
    for (int j = 0; j <= 50; j++) {
        random_string[j][0] = j;
    }
...
int main(int argc, char* argv[]) {
	...
	return_val = connect(data_socket, (const struct sockaddr *) &addr, sizeof(addr));
	...
	if(argc > 1) {
		...
		while(1) {
			...
			for(int i = lastIdx; i < lastIdx + 5; i++) {
				return_val = write(data_socket, randStr[i], strlen(randStr[i]) + 1);
				if(return_val == -1) {
			...
			return_val = read(data_socket, buffer, sizeof(buffer));
			if(return_val == -1) {
				perror("read");
				exit(EXIT_FAILURE);
			}
